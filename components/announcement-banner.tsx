...
const bannerInfos: BannerInfo[] = [
  {
    id: "customer-support",
    desktopText: 'Replexify reduces first-response time by 90% — "Our support team can now focus on complex issues"',
    mobileText: '90% faster response times with Replexify',
    linkText: "See Results",
    linkUrl: "#testimonials",
  },
  {
    id: "ai-automation",
    desktopText: 'AI-powered replies that sound human — "Customers can\'t tell the difference between AI and human support"',
    mobileText: 'AI replies that sound human',
    linkText: "Learn More",
    linkUrl: "#features",
  },
]
    ...
    <div className="fixed top-0 left-0 right-0 z-[60] bg-black border-b border-white/10">
      ...
      >
        ...
        >
          ...
          <div
            className={`w-1 h-4 bg-gradient-to-b from-cyan-400 via-orange-400 to-pink-400 rounded-full transition-all duration-300 ease-out motion-reduce:transition-none ${isTransitioning ? "scale-110 shadow-lg shadow-cyan-400/50" : "scale-100"
              }`}
          ></div>
          ...
          <div className="absolute right-16 hidden sm:flex items-center gap-1.5">
            {bannerInfos.map((_, index) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-500 ease-out motion-reduce:transition-none ${index === currentBannerIndex ? "bg-white w-6 shadow-sm" : "bg-white/30 hover:bg-white/50 w-1.5"
                  }`}
                style={{
